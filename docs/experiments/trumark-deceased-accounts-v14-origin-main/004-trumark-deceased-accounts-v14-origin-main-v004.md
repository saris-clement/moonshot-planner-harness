...
| Variant | trumark-deceased-accounts-v14-origin-main-v004 |
| Parent | none |
| Round | 0 |
| Status | failed |
...
| Patch | `/Users/cflodrops/Documents/dev/playground/test-bootstrap/bootstrap/services/ainative-planner-eval-harness/.data/artifacts/trumark-deceased-accounts-v14-origin-main/trumark-deceased-accounts-v14-origin-main-v004/variant.patch` |
| Image | `ainative-planner-eval:trumark-deceased-accounts-v14-origin-606b2e0431-4` |
...
Answer:
> Use the production deployment environment `trumark-live`. The exact SymXchange base URL/region and secret are deployment-provided; source only calls the relative Saris proxy endpoint `/api/lo_systems/symitar/accounts/with-children-select-fields`. Provision read-only access for member/account and name data, Tracking 50/51/52/53 death and recorded-figure data, deposits/shares and transactions, loans and related transactions/tracking/names/transfers, and Visa/PSCU external loans and tracking. Grant no Symitar write permissions. Note that source requests all loan fields and applies no mortgage/non-mortgage filter, so any consumer-loan-only restriction must be enforced by deployment credentials or the proxy.

Evidence:
- tools/deploy/config.json:29-35 deploys TruMark Deceased Accounts to `trumark-live`.
- src/customers/trumark/deceased-accounts/stages/resolve.ts:81-87 performs the account lookup through `SymitarClient.getAccountSelectFields`.
- src/modules/shared/api/symitar/client.ts:42-43,89-126 uses the relative account-select proxy and requests account, loan, loan tracking/name/transfer/transaction, name, account tracking, share/transaction, and external-loan/tracking fields.
- src/customers/trumark/deceased-accounts/stages/resolve.ts:4-10 identifies Tracking 50/51/52/53 usage for death, shares, loans, and cards.
...
| Current measured facts | `observed_durable` | `/Users/cflodrops/Documents/dev/playground/test-bootstrap/bootstrap/services/ainative-planner-eval-harness/.data/artifacts/trumark-deceased-accounts-v14-origin-main/trumark-deceased-accounts-v14-origin-main-v004/deceased-account/facts.json` | unavailable |
